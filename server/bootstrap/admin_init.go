...
func initMenu(app *Application, roleID string) {
	...
	app.DB.Menu.Create().SetParentID(menuMgmt.ID).SetName("创建菜单").SetSequence(0).SetType("button").SetPath("").SetIcon("PlusCircle").SetIsFrame(false).SetVisible("show").SetPermissions("system:menu:add").AddAPIResourceIDs("POST:/api/v1/system/menu").SetStatus("active").Save(ctx)
	app.DB.Menu.Create().SetParentID(menuMgmt.ID).SetName("编辑菜单").SetSequence(0).SetType("button").SetPath("").SetIcon("Edit").SetIsFrame(false).SetVisible("show").SetPermissions("system:menu:edit").AddAPIResourceIDs("PUT:/api/v1/system/menu/:id").SetStatus("active").Save(ctx)
	app.DB.Menu.Create().SetParentID(menuMgmt.ID).SetName("删除菜单").SetSequence(0).SetType("button").SetPath("").SetIcon("Trash2").SetIsFrame(false).SetVisible("show").SetPermissions("system:menu:delete").AddAPIResourceIDs("DELETE:/api/v1/system/menu/:id").SetStatus("active").Save(ctx)
